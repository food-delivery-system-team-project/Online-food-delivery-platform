import { motion } from "motion/react";
import img1 from "../Images/slides/1.png";
import img2 from "../Images/slides/2.png";
import img3 from "../Images/slides/3.png";
import img4 from "../Images/slides/4.png";
import img5 from "../Images/slides/5.png";
import img6 from "../Images/slides/6.png";
import img7 from "../Images/slides/7.png";
import img8 from "../Images/slides/4.png";
import img9 from "../Images/slides/5.png";
import img10 from "../Images/slides/6.png";
import img11 from "../Images/slides/7.png";
import img12 from "../Images/slides/4.png";
import img13 from "../Images/slides/5.png";

import Navbar from "../Components/Navbar";
import Footer from "./Footer";
import FoodCard from "../Components/Card/FoodCard";
import API from "../api/fetchApi";
import { useDispatch } from "react-redux";
import { setFood } from "../Features/likeSlice";

import { useEffect, useState } from "react";
import { Link } from "react-router-dom";

const Home = () => {
  const dispatch = useDispatch();
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchFoods = async () => {
  try {
    const res = await API.get("/foods", {
      params: {
        page: 1,
        limit: 10,
      }
    });

    setData(res.data.foods);        // ✅ FIX THIS
    dispatch(setFood(res.data.foods)); // keep redux in sync
    setLoading(false);

  } catch (err) {
    console.log(err.response?.data || err.message);
    setLoading(false);
  }
};

  useEffect(() => {
    fetchFoods();
  }, []);

  return (
    <>
      <Navbar />

      <div className='h-screen w-full bg-[#ff6e4a] overflow-hidden relative'>

        {/* TEXT */}
        <div className="absolute top-75 left-30">
          <img
            className="absolute top-50 left-0 w-40 h-50"
            src="./src/Images/home-bag-logo.png"
            alt=""
          />
          <h1 className="text-8xl font-bold text-white">
            Delicious Food, Delivered Fast
          </h1>
          <p className="absolute top-60 left-50 text-3xl text-white">
            Experience the joy of food delivery with us.
          </p>

          <Link to="/explore">
            <motion.button
              className="absolute top-85 left-50 bg-white text-[#ff6e4a] font-bold px-5 py-3 rounded-lg"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              Order Now
            </motion.button>
          </Link>
        </div>

        {/* ROTATING IMAGES */}
        <motion.div
          className="image top-100 right-[-400px]"
          animate={{ rotate: 360 }}
          transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
        >
          {[img1, img2, img3, img4, img5, img6, img7, img8, img9, img10, img11, img12, img13].map((img, index) => (
            <div key={index} className="item" style={{ "--i": index + 1 }}>
              <img src={img} alt="" />
            </div>
          ))}
        </motion.div>

        {/* CTA BOX */}
        <div className="absolute bottom-0 w-full flex justify-center">
          <motion.div
            whileHover={{ y: -20 }}
            className="h-60 w-400 rounded-4xl bg-white/80 flex justify-between p-10 items-center"
          >
            <h1 className="text-3xl font-bold text-black/60">
              Explore The Delicious Meal Here
            </h1>

            <Link to="/explore">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-[#ff6e4a] text-white font-bold px-5 py-3 rounded-lg"
              >
                See All
              </motion.button>
            </Link>
          </motion.div>
        </div>
      </div>

      {/* FOOD LIST */}
      <div className="min-h-screen w-full bg-[#faf1ee] flex flex-wrap gap-5 p-10">
        {loading ? (
          <p>Loading...</p>
        ) : data.length === 0 ? (
          <p>No foods found</p>
        ) : (
          data.map((item) => (
            <FoodCard key={item._id} food={item} />
          ))
        )}
      </div>

      <Footer />
    </>
  );
};

export default Home;