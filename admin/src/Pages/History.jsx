  ...
  const filteredHistory = useMemo(() => {
    ...
    return history.filter((entry) => {
      const matchesCategory =
        activeCategory === "All" || entry.section === activeCategory;
      const matchesQuery =
        !needle ||
        [
          entry.title,
          ...
          .includes(needle);
      const matchesDate =
        !selectedDate || entry.timestamp?.slice(0, 10) === selectedDate;
  ...
  const groupedHistory = useMemo(() => {
    return categoryMeta.reduce((acc, category) => {
      if (category.key === "All") {
        ...
        acc[category.key] = filteredHistory.filter(
          (entry) => entry.section === category.key,
        );
      }
      return acc;
    }, {});
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-500 dark:text-slate-400">
            Track food, order, customer, profile, and settings changes in one
            place.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <label className="relative min-w-[220px]">
            <Search
              size={16}
              className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-500 dark:text-slate-400"
            />
            <input
              value={query}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {loading
          ? Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="card h-52 animate-pulse bg-ink-100/50 dark:bg-slate-800/70"
              />
            ))
          : categoryMeta.map(({ key, label, icon: Icon }) => {
                  ...
                  <div className="space-y-3">
                    {items.length ? (
                      items.map((entry) => (
                        <article
                          key={entry.id}
                          className="rounded-2xl border border-ink-100 bg-ink-50/70 p-3.5 dark:border-slate-700 dark:bg-slate-800/70"
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span
                              className={`badge ${sectionStyles[entry.section]}`}
                            >
                              {entry.section}
                            </span>
                          </div>
